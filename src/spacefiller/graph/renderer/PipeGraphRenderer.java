...
//        List<Node> connections = new ArrayList<>(a.end.getNeighbors());
//
//        // Pick a new edge from the filtered list and start animating it.
//        Node end = connections.get((int) Math.floor(connections.size() * Math.random()));
//        if (edges.size() < maxPoints) {
//          Node start = a.end;
//          AnimationInfo newAnimation = new AnimationInfo(start, end, (float) Math.random() * 3 + 1);
...
//    boolean finished;
//
//    public AnimationInfo(Node start, Node end, float speed) {
//      this.start = start;
//      this.end = end;
//      this.speed = speed;
